...
class Company {
   ...
   * */
	...
	/** Find companies filtered by one or more parameters.
   *
   ...
   * */
	...
	static async findFiltered(params) {
		const { whereCols, values } = sqlForCompFilter(params);
		const companiesRes = await db.query(
			`SELECT handle,
                  name,
                  description,
		...
		if (companiesRes.rows.length === 0) {
			return { msg: 'No companies match the search criteria' };
		}
		return companiesRes.rows;
	}
		...
		const companyRes = await db.query(
			`SELECT c.handle,
                  ...
                  COALESCE(json_agg(json_build_object(
                      ...
                      'title', j.title,
                      'salary', j.salary,
                      'equity', j.equity)) FILTER (WHERE j.id IS NOT NULL), '[]' )AS jobs
           FROM companies AS c
           LEFT JOIN jobs AS j
